...
interface DeleteExamQuestionUseCaseRequest {
  plantId?: string
  journeySlug: string
  moduleSlug: string
  examSlug: string
...
export class DeleteExamQuestionUseCase {
  ...
  async execute({
    ...
    examSlug,
    questionId,
  }: DeleteExamQuestionUseCaseRequest) {
    if (!plantId) {
    ...
    const questionDeleted = await this.examQuestionsRepository.delete(question.id)

    if (!questionDeleted) {
      throw new GenericDeletingError('Não foi possível deletar a questão.')
    }
